...
import { ArrowLeft, UploadCloud, FileText, CheckCircle2, AlertCircle, ShieldAlert } from "lucide-react";
...
export default function AdminUploadPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (currentUser) => {
      ...
      const roleValue = roleDoc.exists() ? roleDoc.data().role || "Student" : "Student";
      setRole(roleValue);

      if (String(roleValue).toLowerCase() !== "admin") {
  ...
  async function handleUpload(event) {
    ...
    if (!file || !user || !isAdmin || uploading) {
      return;
    }
    ...
    try {
      ...
      setStatus(`Upload successful. Stored ${payload.chunksStored} chunks.`);
      setFile(null);
    } catch (err) {
      ...
      <header className="sticky top-0 z-10 border-b border-gray-200 bg-white/80 px-6 py-4 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-[18px] font-bold text-[#1E1E1E]">Admin Dashboard</h1>
              <p className="text-[13px] text-[#6B6B6B]">Knowledge Base Management</p>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-4xl p-6 py-10">
        <div className="overflow-hidden rounded-[20px] bg-white shadow-[0_15px_40px_rgba(0,0,0,0.08)]">
          ...
          <div className="border-b border-gray-100 bg-gradient-to-r from-[#F4F2FA] to-white p-8">
            ...
            <h2 className="text-[24px] font-bold text-[#1E1E1E]">Upload Documents</h2>
            <p className="mt-2 max-w-xl text-[14px] leading-relaxed text-[#6B6B6B]">
              Upload university PDFs (syllabi, policies, schedules). The text will be automatically chunked, embedded, and added to the vector database for Amity AI to use.
            </p>
          </div>
          ...
          <div className="p-8">
            <form onSubmit={handleUpload} className="space-y-6">
              ...
              {/* Status Messages */}
              {status && (
                <div className="flex items-center gap-3 rounded-[12px] bg-green-50 p-4 text-[14px] text-green-700">
